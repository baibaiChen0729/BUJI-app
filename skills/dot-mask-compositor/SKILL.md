...
| 案例 | 文件 | 用了哪些能力 |
|---|---|---|
| 步集 · 实时模板相机 v3 | `/Users/chenwenjing2/Desktop/codewize/live-template-camera.html` | **v2 容器模型** / **固定预览容器 430×572（防控件位移）** / 4 形状（圆·雨滴·五角星·喵）/ 自定义填充色 / 裁切占比 / 横竖分割 / swap |
...
> 这是 v2 可变容器的**必需外壳**。因为 v2 里画面尺寸会随「占比 / 横竖」实时变化，
> 如果让画面直接参与 flex 布局，它一变大变小，**下方所有控件（方向/形状/参数/颜色/变焦/快门）就会跟着上下跳动**。
> 解决办法：套一层**固定尺寸容器**，让画面在容器内变化，容器高度恒定。

**对齐 Figma（`v2WOCWYFzvdDMsumz2cDoG / 347:1830`）的容器结构：**
```
Camera 430×932
  └─ Frame 2116747366  430×572   ← 固定预览容器（高度恒定，绝不参与布局收缩）
       └─ Corner Markers 406×572 (clipsContent)  ← 内层裁切区 + 四角 L 标
            └─ 照片 / 波点合成画面   ← 尺寸在这里变，只在容器内部
```
...
```
```css
.camera-stage { flex-shrink: 0; }          /* 不被 flex 压缩 */
.preview-container {
  width: 430px; height: 572px;             /* ← 固定！对齐 Figma 外层 frame */
  position: relative; overflow: hidden;    /* 超出画面裁掉 */
  flex-shrink: 0;
  display: flex; align-items: center; justify-content: center;  /* 画面居中 */
}
.canvas-wrapper {
  position: relative;                       /* 给 fps/水印等绝对定位子元素锚点 */
  overflow: hidden; border-radius: 8px;
  flex-shrink: 0;                           /* 画面尺寸由 JS 设，容器不缩它 */
}
```
```js
// updateCanvasSize() 只改 canvas-wrapper 的宽高即可，居中交给父容器 flex：
wrapper.style.width  = W + 'px';
wrapper.style.height = H + 'px';
```

**三条铁律（踩过的坑）：**
1. **画面居中必须用父容器 `flex`，不要用 `.canvas-wrapper { transform: translate(-50%,-50%) }`**
   —— 因为翻转动画会执行 `wrapper.style.transform = 'rotateY(...)'`，**会把居中 transform 覆盖掉**，导致画面偏移。flex 居中由父级负责，wrapper 自己的 transform 怎么变都不影响。
2. **容器宽高写死**（如 430×572），`flex-shrink: 0`，别让它参与任何 flex 伸缩。
3. **容器 `overflow: hidden`**，画面变大时超出部分裁掉、变小时四周留白，视觉稳定。

**自检**：拖占比滑块 / 切横竖 / 翻转前后，`preview-container.getBoundingClientRect()` 的 `height` 与 `bottom` 应**恒定不变**，下方首个控件的 `top` 也应不变。
